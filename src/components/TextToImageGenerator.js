...
const TextToImageGenerator = () => {
  ...
  const [buttonText, setButtonText] = useState('Random Avatar Generate'); // State for button text

  // useEffect hook to update button text based on input text
  useEffect(() => {
    setButtonText(text ? 'Generate Avatar' : 'Random Avatar Generate');
  ...
  return (
    <div className="container mx-auto p-4">
      <h1 className="text-3xl font-bold text-center mb-4 text-white" style={{ fontFamily: 'Poppins, sans-serif' }}> Generate Your free Avatar</h1>
  ...
  <div key={index} className="relative mb-4">
    <img src={image} alt={`Generated ${index}`} className="max-w-full rounded shadow-lg" />
    <a 
      href={image} 
      download={`generated-image-${index}.png`} // Specify the file name for the download
      className="absolute bottom-2 left-2 bg-indigo-500 hover:bg-indigo-400 text-white font-bold py-1 px-3 rounded"
    >
      Download
    </a>
  </div>
